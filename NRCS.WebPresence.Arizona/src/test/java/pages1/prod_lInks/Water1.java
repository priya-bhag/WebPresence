...
public class Water1 {
	...
	@FindBy(xpath = ".//*[@id='selectedTree']/p/a")
	static List<WebElement> Table_MainMenuProd;
	...
	@FindBy(xpath = ".//*[@id='firstElement']/p/a")
	static List<WebElement> Table_MainMenuTest;
	...
	public static List<String> LeftMenuArrayProd = null;

	public static List<String> LeftMenuArrayTest = null;
	...
	public static void validateURL() throws IOException, InterruptedException {
		...
		Logger log = Logger.getLogger("Water1");
		 ...
		 * 
		 * log.info("No of main menu elements in Prod are " +
		 * LeftMainArrayProd.size());
		 * 
		 * log.info("\t");
		 */

		List<String> LeftMenuArrayProd = new ArrayList<String>();

		List<WebElement> LeftMenuArrayProd_e = Table_MainMenuProd;

		for (int i = 0; i <= LeftMenuArrayProd_e.size() - 1; i++) {

			String text = LeftMenuArrayProd_e.get(i).getText();

			LeftMenuArrayProd.add(text);

		}

		/*
		 * log.info("Main menu in Prod is");
		 * 
		 * log.info(LeftMenuArrayProd);
		 */
		...
		driver.get(DataProviderFactory.getConfig().getApplicationUrl3()
				+ DataProviderFactory.getExcel().getdata(0, 81, 1));

		Helper.capturescreenshot(driver, "WaterAZ_Test");
...
log.info("heading of both urls are  not same := "+ " Prod : " +heading1 + ", Test: " +heading2 );		}
		...
		List<String> LeftMenuArrayTest = new ArrayList<String>();

		List<WebElement> LeftMenuArrayTest_e = Table_MainMenuTest;

		for (int i = 0; i <= LeftMenuArrayTest_e.size() - 1; i++) {

			String text = LeftMenuArrayTest_e.get(i).getText();

			LeftMenuArrayTest.add(text);

		}

		/*
		 * log.info("Main menu in Test is");
		 * 
		 * log.info(LeftMenuArrayTest);
		 */
		...
		Boolean value1 = Arrays.equals(LeftMenuArrayProd.toArray(), LeftMenuArrayTest.toArray());

		try {

			if (!value1 == true) {

				log.info("MAIN menu on  both Env: are not same");
			}
		} catch (Exception e) {
