...
public class LocalServiceCenters1 {
	...
	public static void validateProd() {
		...
		if (IsMApPresentProd == true) {
			...
			log.info("map is present on the page right side : Prod");
		} else {
		...
		driver.get(DataProviderFactory.getConfig().getApplicationUrl3() + "mo/contact/local/");
		...
		if (IsMApPresentTest == true) {
			...
			log.info("map is present on the page right side : Test");
		} else {
