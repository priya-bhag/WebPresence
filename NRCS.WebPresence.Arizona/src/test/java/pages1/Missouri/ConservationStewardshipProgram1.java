...
public class ConservationStewardshipProgram1 {

	static WebDriver driver;

	public ConservationStewardshipProgram1(WebDriver Idriver) {
	...
	/*
	 * @FindBy(xpath=".//*[@id='overview']/p[1]/img") static WebElement
	 * Image1Prod;
	 */
	...
	/*
	 * @FindBy(xpath=
	 * ".//*[@id='layoutContainers']/div/table/tbody/tr/td/table/tbody/tr/td/div/section/div[2]/div[2]/div[2]/p[2]/img")
	...
	public static void validateURL() {
		...
		Logger log = Logger.getLogger("ConservationStewardshipProgram1");
		...
		// mageArray2Prod =image_check(Image2Prod);

		// Test Methods

		driver.get(DataProviderFactory.getConfig().getApplicationUrl3() + "mo/programs/financial/csp/");

		Helper.capturescreenshot(driver, "ConservationStewardshipProgramIN_Test");
		...
		/*
		 * Boolean value
		 * =Arrays.equals(Image1ArrayProd.toArray(),Image1ArrayTest.toArray());
		 * 
		 * 
		 * try{
		 * 
		...
		try {
			if (count == count1) {
				...
				log.info("Both URLs has got same number of internal and external links");
			}

			else {

				log.info("No of links doesnt match in both URLs");
			}
	 ...
	 * , ImageFile);
	 * 
	 * if (!ImagePresent) { log.info("Image not displayed."); } else {
	 ...
	 * 
	 * int width =dimensions.width; int height=dimensions.height;
	 * 
	 * //log.info("height is " + dimensions.height);
	 * 
	 * Point point =image1Prod2.getLocation();
	 * 
	 * int Xposition=point.x; int Yposition=point.y;
	 * 
	 * //ImageArray.add(width); //ImageArray.add(height);
	 * ImageArray.add(Xposition); ImageArray.add(Yposition);
	 * 
	 * 
	 * log.info("X position and Y position of image "+ImageArray); return
	 * ImageArray;
	 * 
	 * //log.info("x position is " +point.x); //log.info("y position is "
	 * +point.y);
	 * 
	 * 
	 * }
