...
public class ConservationPlanning1 {
	...
	public static void validateURL() {
		...
		Logger log = Logger.getLogger("ConservationPlanning1");
		...
		driver.get(DataProviderFactory.getConfig().getApplicationUrl3() + "az/technical/cp/#");

		Helper.capturescreenshot(driver, "ConservationPlanningAZ_Test");
		...
		try {
			...
			if (d == false) {
				log.info("image6 of both urls are not same");
		...
		try {
			...
			if (f == false) {
				log.info("image7 of both urls are not same");
		...
		try {
			...
			if (g == false) {
				log.info("image8 of both urls are not same");
		...
		try {
			...
			if (f1 == false) {
				log.info("image9 of both urls are not same");
		...
		try {
			...
			if (f2 == false) {
				log.info("image10 of both urls are not same");
