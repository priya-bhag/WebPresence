...
import utility.Find_brokenLinks;
import utility.Helper;

public class WatershedRehabilitation1 {

	static WebDriver driver;
	static Logger log = Logger.getLogger("WatershedRehabilitation1");

	public WatershedRehabilitation1(WebDriver Idriver) {
		this.driver = Idriver;
	}
	...
	@FindBy(xpath = ".//*[@id='overview']/h4/img")
	static WebElement Image1Prod;

	@FindBy(xpath = ".//*[@id='overview']/table/tbody/tr/td[2]/h3/img")
	...
	public static void validateURL() {
		...
		Logger log = Logger.getLogger("WatershedRehabilitation1");
		...
		driver.get(DataProviderFactory.getConfig().getApplicationUrl3() + "az/programs/planning/wr/");

		Helper.capturescreenshot(driver, "WatershedRehabilitationAZ_Test");
		...
		try {
			...
			log.error("Error in image loading " + E.toString());
		}
		...
		/*
		 * if( NewcontentProd.equalsIgnoreCase(NewcontentTest )){
		 * log.info("The contents in both Env: are same"); } else{
		 * log.info("The contents in both Env: are not same"); } log.info("\t");
		 */
		 ...
		 * String text=linkUrl+" - "+httpURLConnect.getResponseMessage();
		 * 
		 * if(text.contains("Found")||text.contains("Moved Permanently")||text.
		 * contains(" Forbidden")){
		 * 
		 * } } //
		 * log.info("No of links with response code other than OK are "+count2);
		 * 
		 ...
		 * " - "+ HttpURLConnection.HTTP_NOT_FOUND;
		 * 
		 * if (text.contains("HTTP_NOT_FOUND")){ // count3=count3+1; }
		 */
