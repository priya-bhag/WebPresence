...
public class SnowSurvey1 {
	...
	@FindBy(xpath = ".//*[@id='table1']/tbody/tr/td[1]/img")
	static WebElement Image2Prod;

	@FindBy(xpath = ".//*[@id='table1']/tbody/tr/td[2]/h5[1]/img")
	...
	@FindBy(xpath = ".//*[@id='layoutContainers']/div/table/tbody/tr/td/table/tbody/tr/td/div/section/div[2]/div[2]/div[2]/table[1]/tbody/tr[1]/td[1]/img")
	...
	public static void validateProd() {
		...
		for (int j = 0; j < tableItems_Prod.size() ; j++) {
			...
			LinksArrayProd.add(link_Prod);
			count = count + 1;

			// verifyLinkActive(link_Prod);
			try {

				URL url1 = new URL(link_Prod);
		...
		log.info("No of links in Prod are " + count);

		log.info("No of links with href null Prod are " + c1);

		log.info("No of email  links in Prod are " + c2);
		...
		for (int i = 0; i < Left_SubMenuElementProd.size() ; i++) {
		...
		driver.get(DataProviderFactory.getConfig().getApplicationUrl3() + "az/snow/");
